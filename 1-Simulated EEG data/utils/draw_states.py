...
def draw_state_blocks_for_eeg(state, figsize=(4, 2), colorbar_fraction=0.025, 
                     tmin=0.0, tmax=1.0, colorbar_ticks=[], n_clusters=0, 
    ...
    colors.extend( [tab[1], tab[3], tab[5], tab[7], tab[9], tab[11], tab[13], tab[17], tab[19]] )
    colors = colors[:n_clusters]
    my_cmap = LinearSegmentedColormap.from_list('my_cmap', colors[:n_clusters], N=n_clusters)
    ...
    # x = np.linspace(0, int(tp), num=9, endpoint=True)
    x = np.arange(0, int(tp-1), step=50) + 25
    xticks = np.around( np.linspace(tmin, tmax, num=9, endpoint=True), decimals=1)
    ...
    if current_cluster > -1:
        displacement = int(451*current_cluster)
        rie_center_index = []

        riemann_dist_symmetry = np.zeros((451, 451))
        for i in range(451):
            for j in range(i+1, 451):
        ...
        for clu in list(set(state)):
            ...
            # rie_center_index.append( np.argwhere(state==clu)[index][0] )
...
def draw_grand_average_topo(epo_list, epochs, n_rows=3, draw_separate=False, cmap='turbo', colorbar=False):
    ...
    if not draw_separate:
        f_evoked.plot_topomap(
                ...
                time_unit='s',
                cmap=cmap,
                vlim=(np.percentile(epo_list, 1)*1e6, np.percentile(epo_list, 99)*1e6),#(-3, 3)
                show=True,
                colorbar=False,
                size=0.7,
                # res=1000,
