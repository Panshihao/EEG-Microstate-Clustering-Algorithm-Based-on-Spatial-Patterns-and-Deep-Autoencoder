...
def prewhiten(X):    
    ...
    # Rxx
    L = X.shape[1] - 1
    Cov = (1.0/L) * np.dot(Xc, Xc.transpose())
    V, E = np.linalg.eig(Cov)


    Lambda = np.zeros((V.shape[0], V.shape[0]))
    for index, v in enumerate(V):
        Lambda[index, index] = v

    Lambda_inv = np.linalg.inv(Lambda)

    Lambda_inv_sqrt = np.zeros_like(Lambda_inv)

    for i in range(Lambda_inv.shape[0]):
        Lambda_inv_sqrt[i,i] = np.sqrt(Lambda_inv[i,i])

    # W=lambda^(-1/2) U^T
    W = np.dot(Lambda_inv_sqrt, E.transpose())
    
    return W, np.dot(W, Xc), Cov
...
def jd(X, eps=1.0e-6, random_order=True):
    ...
    counter = 0
    off_val = []
    while keep_going and counter<100:
...
def sobi(X, num_lags=None, eps=1.0e-6, random_order = True):
    ...
    # A = Sig_x B.T Sig_\hat{s}
    L = S.shape[1] - 1
    CovS = (1.0/L) * np.dot(S, S.transpose())
    CovXw = (1.0/L) * np.dot(Xw, Xw.transpose())
    A = np.dot( np.dot(CovXw, B.T), CovS )

    return S, A, B, Q



def sojd(X, num_lags=None, eps=1.0e-6, random_order = True):
    """
    second-order joint diagonalization of Eigen matrix
