...
class ClusteringLayer(nn.Module):
    def __init__(self, n_clusters=8, hidden=16, cluster_centers=None, alpha=1.0):
...
class DEC(nn.Module):
    def __init__(self, n_clusters=8, autoencoder=None, hidden=16, cluster_centers=None, alpha=1.0):
    ...
    def visualize(self, epoch, x, file_path_prefix):
        ...
        x = x.cpu().numpy()[:1000]
        x_embedded = TSNE(n_components=2, random_state=2, n_jobs=-1).fit_transform(x)
...
def train(**kwargs):
    ...
    # ============K-means=======================================
    kmeans = KMeans(n_clusters=n_cluster, random_state=3, n_init=int(n_cluster*100)).fit(features)
